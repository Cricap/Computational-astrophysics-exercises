...
integer*8 :: sdr, Num, stamp, ncicli, index, posmax, Num2
...
do i = 1, 10
	open(labels(i), file = trim("results_test_")//trim(times_string(i))//trim(".dat"))
 enddo
...
ts=1.
te=1.
open(70, file='Sedovcool.dat')
open(80, file='Energycool.dat')

open(90, file='Luminositycool.dat')
        ncicli=0
 ...
 if (t >= times(Num2)*1.d4*yr .and. Num2 < 11) then

	do i = 1, N - 1
	   write(labels(Num2), 1002) xa(i)/cmpc, xb(i)/cmpc, d(i)/d0, Temp(i), P(i), v(i)
	end do
